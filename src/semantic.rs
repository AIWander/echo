...
impl SemanticIndex {
    ...
    pub async fn search(
        ...
        let mut stmt = self
            .conn
            .prepare("SELECT d.id, d.path, d.title, d.content FROM documents d")?;
        ...
        for (doc_id, path, title, content) in docs {
            let mut emb_stmt = self
                .conn
                .prepare("SELECT value FROM embeddings WHERE doc_id = ?1 ORDER BY dim")?;
...
fn should_index(path: &Path) -> bool {
    ...
    if s.contains(".tantivy_index") || s.contains(".semantic_index") {
        return false;
    }
...
pub async fn handle_semantic_search(args: &Value) -> Result<Value> {
    ...
    let model = args.get("model").and_then(|m| m.as_str()).ok_or_else(|| {
        anyhow::anyhow!(
            "Missing 'model' parameter — specify the embedding model used at reindex time"
        )
    })?;
...
pub async fn handle_semantic_reindex(args: &Value) -> Result<Value> {
    ...
    let test_embedding = probe_client
        .embed("dimension probe", model)
        .await
        .map_err(|e| {
            anyhow::anyhow!(
                "Failed to connect to Ollama with model '{}': {}. Is 'ollama serve' running?",
                model,
                e
            )
        })?;
        ...
        .filter(|e| e.path().is_file() && should_index(e.path()))
    {
        ...
        if let Ok(content) = std::fs::read_to_string(entry.path()) {
            ...
            if chunks_batch.len() >= 100 {
                index
                    .add_chunks_batch(std::mem::take(&mut chunks_batch), model)
                    .await?;
            }
        }
    }
